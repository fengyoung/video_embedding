...
def image_retrieval_by_feat(src_image_feat_vec, image_feats, sim_type): 
	sim_vals = []	
	for imf in image_feats:
		sim = image_sim_match(src_image_feat_vec, imf[2], sim_type)
		sim_vals.append([imf[0], imf[1], sim])
	sim_vals = sorted(sim_vals, cmp = lambda x, y: cmp(x[2], y[2]), reverse = True)
	return sim_vals	
	
	
def run_image_sim_match(graph_file, image, image_feats, sim_type): 
	image2vec.create_graph(graph_file)
	with tf.Session() as sess:
		if gfile.Exists(image) == True:
			image_data = gfile.FastGFile(image, 'rb').read()
			feature_tensor = sess.graph.get_tensor_by_name('pool_3:0')
			feat_vec = sess.run(feature_tensor, {'DecodeJpeg/contents:0': image_data})
			feat_vec = np.squeeze(feat_vec)
			
			sim_vals = image_retrieval_by_feat(feat_vec, image_feats, sim_type)
...
if __name__ == '__main__':
	...
	run_image_sim_match(sys.argv[1], sys.argv[3], image_feats, sys.argv[4])
